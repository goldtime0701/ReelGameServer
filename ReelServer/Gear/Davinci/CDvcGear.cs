...
{
    public partial class CDvcGear : CBaseGear
    {
        private CDVCPrizeInfo m_prizeInfo;                  //잭팟정보
        private List<CDVCScoreInfo> m_lstPrizeInfo;      //잭팟정보리스트
        private List<CDVCScoreInfo> m_lstScoreInfo;

        private List<CDVCScoreInfo> m_lstReturnGift;
        private CDVCScoreInfo m_clsSendScoreInfo;
        ...
        public CDvcGear() : base()
        {
            m_nSpeedCash = 100;
            m_lstScoreInfo = new List<CDVCScoreInfo>();
            m_lstPrizeInfo = new List<CDVCScoreInfo>();
            m_lstReturnGift = new List<CDVCScoreInfo>();
        ...
        public override void MakeNormalScorelRoll(int nAddCash, bool bFlag = true)
        {
            ...
            List<CDVCScoreInfo> lstDvcScoreInfo = new List<CDVCScoreInfo>();
            ...
            while (nTempCash > 0)
            {
                ...
                List<CDVCScoreInfo> lstScoreInfo = new List<CDVCScoreInfo>();
                for (int i = 0; i < lstScore.Count; i++)
                {
                    int nDstScore = lstScore[i]; //목표점수
                    CDVCScoreInfo scoreInfo = MakeNormalScore(nDstScore);
                    lstScoreInfo.Add(scoreInfo);
                }

                while (lstScoreInfo.Count > 0)
                {
                    CDVCScoreInfo scoreInfo = CGlobal.RandomSelect(lstScoreInfo);
                    lstDvcScoreInfo.Add(scoreInfo);
            ...
            int nEmptyCount = nRollCount - lstDvcScoreInfo.Count;
            for (int i = 0; i < nEmptyCount; i++)
            {
                CDVCScoreInfo scoreInfo = MakeEmptyRoll();
                int nRnd = RND.Next(5);
                if (nRnd == 3)
                    lstDvcScoreInfo.Add(scoreInfo);
                else
                {
                    int nIndex = RND.Next(lstDvcScoreInfo.Count);
                    lstDvcScoreInfo.Insert(nIndex, scoreInfo);
                }
            }
            ...
            m_lstScoreInfo.AddRange(lstDvcScoreInfo);
        }
        ...
        //점수로부터 일반점수정보 만들기
        private CDVCScoreInfo MakeNormalScore(int nScore)
        {
            if (nScore == 0)
                return MakeEmptyRoll();

            //점수테이블리스트를 만든다.
            List<CDVCScoreTable> lstMatchScoreInfo = m_lstScoreTable.FindAll(value => value.m_nScore == nScore);
            CDVCScoreTable scoreTable = CGlobal.RandomSelect(lstMatchScoreInfo);
            CDVCScoreInfo scoreInfo = ConvertScoreTableToScoreInfo(scoreTable);
            m_nOverCash += scoreInfo.m_nScore - nScore;

            return scoreInfo;
        }

        private CDVCScoreInfo ConvertScoreTableToScoreInfo(CDVCScoreTable scoreTable)
        {
            CDVCScoreInfo scoreInfo = MakeScoreInfoFromScoreTable(scoreTable);
            return scoreInfo;
        }

        private CDVCScoreInfo MakeScoreInfoFromScoreTable(CDVCScoreTable scoreTable)
        {
            CDVCScoreInfo scoreInfo = new CDVCScoreInfo();
            ...
            for (int i = 0; i < scoreTable.m_nlstTile.Length; i++)
            {
                ...
                else if (nTile == ANNY)
                {
                    ...
                    if (i == 0 || i == 3)
                        nTile = CGlobal.RandomSelect(lstTiles.FindAll(value => value != DAYA));
                    else
            ...
            for (int nRow = 0; nRow < 3; nRow++)
            {
                ...
                while (true)
                {
                    nTile = CGlobal.RandomSelect(m_lstTiles.ToList().FindAll(value => value != DAYA));
            ...
            for (int nRow = 0; nRow < 3; nRow++)
            {
                ...
                while (true)
                {
                    ...
                    if (lstLine0[nRow] == nTile && (nTile == JOKE || nTile == DAYA || nTile == STAR || nTile == TAGT || nTile == SEVN))
                        continue;
                    if (lstLine0[nRow] == JOKE && (nTile == DAYA || nTile == STAR || nTile == TAGT || nTile == SEVN))
                        continue;
                    if (nTile == JOKE && (lstLine0[nRow] == STAR || lstLine0[nRow] == SEVN || lstLine0[nRow] == TAGT || lstLine0[nRow] == DAYA))
                        continue;
            ...
            for (int nRow = 0; nRow < 3; nRow++)
            {
                ...
                while (true)
                {
                    ...
                    if (nTile == lstLine3[nRow] && (nTile == JOKE || nTile == DAYA || nTile == STAR || nTile == SEVN || nTile == TAGT))
                        continue;
                    if (lstLine3[nRow] == JOKE && (nTile == DAYA || nTile == STAR || nTile == SEVN || nTile == TAGT))
                        continue;
                    if (nTile == JOKE && (lstLine3[nRow] == DAYA || lstLine3[nRow] == STAR || lstLine3[nRow] == SEVN || lstLine3[nRow] == TAGT))
                        continue;
        ...
        private CDVCScoreInfo MakeEmptyRoll()
        {
            CDVCScoreInfo scoreInfo = new CDVCScoreInfo();
            int[] nBarCnt = { 0, 0, 0 };
            ...
            while (true)
            {
                ...
                if (m_lstOriginalTile[0][nPos] == DAYA)
                    continue;
            ...
            while (true)
            {
                lstLine3.Clear();
                int nPos = RND.Next(0, 11);
                if (m_lstOriginalTile[3][nPos] == DAYA)
                    continue;
            ...
            while (true)
            {
                ...
                if (lstLine0[0] == nTile && (nTile == JOKE || nTile == DAYA || nTile == STAR || nTile == TAGT || nTile == SEVN))
                    continue;
                if (lstLine0[0] == JOKE && (nTile == DAYA || nTile == STAR || nTile == TAGT || nTile == SEVN))
                    continue;
                if (nTile == JOKE && (lstLine0[0] == STAR || lstLine0[0] == SEVN || lstLine0[0] == TAGT || lstLine0[0] == DAYA))
                    continue;
                ...
                if (lstLine0[1] == nTile && (nTile == JOKE || nTile == DAYA || nTile == STAR || nTile == TAGT || nTile == SEVN))
                    continue;
                if (lstLine0[1] == JOKE && (nTile == DAYA || nTile == STAR || nTile == TAGT || nTile == SEVN))
                    continue;
                ...
                if (nPos >= m_lstOriginalTile[1].Count)
                    nPos = 0;
                nTile = m_lstOriginalTile[1][nPos];
                if (lstLine0[2] == nTile && (nTile == JOKE || nTile == DAYA || nTile == STAR || nTile == TAGT || nTile == SEVN))
                    continue;
                if (lstLine0[2] == JOKE && (nTile == DAYA || nTile == STAR || nTile == TAGT || nTile == SEVN))
                    continue;
            ...
            while (true)
            {
                ...
                if(nTempCnt < 5000)
                {
                    if (nTile == lstLine3[0] && (nTile == JOKE || nTile == DAYA || nTile == STAR || nTile == SEVN || nTile == TAGT))
                        continue;
                    if (lstLine3[0] == JOKE && (nTile == DAYA || nTile == STAR || nTile == SEVN || nTile == TAGT))
                        continue;
                    if (nTile == JOKE && (lstLine3[0] == DAYA || lstLine3[0] == STAR || lstLine3[0] == SEVN || lstLine3[0] == TAGT))
                        continue;
                ...
                if (nTempCnt < 5000)
                {
                    if (nTile == lstLine3[1] && (nTile == JOKE || nTile == DAYA || nTile == STAR || nTile == SEVN || nTile == TAGT))
                        continue;
                    if (lstLine3[1] == JOKE && (nTile == DAYA || nTile == STAR || nTile == SEVN || nTile == TAGT))
                        continue;
                    if (nTile == JOKE && (lstLine3[1] == DAYA || lstLine3[1] == STAR || lstLine3[1] == SEVN || lstLine3[1] == TAGT))
                        continue;
                ...
                if (nTempCnt < 5000)
                {
                    if (nTile == lstLine3[2] && (nTile == JOKE || nTile == DAYA || nTile == STAR || nTile == SEVN || nTile == TAGT))
                        continue;
                    if (lstLine3[2] == JOKE && (nTile == DAYA || nTile == STAR || nTile == SEVN || nTile == TAGT))
                        continue;
                    if (nTile == JOKE && (lstLine3[2] == DAYA || lstLine3[2] == STAR || lstLine3[2] == SEVN || lstLine3[2] == TAGT))
                        continue;
        ...
        public bool SetDvcPrizeCall(int nCont, int nJackAmount)
        {
            if (m_nGearJack > 0)
                return false;
            m_prizeInfo = new CDVCPrizeInfo();
        ...
        public override bool MakePrizeRoll()
        {
            ...
            //보석 출현
            MakePrizeCommand(PRIZE_DAYA);
            if (m_prizeInfo.m_nCont == JACK_DAYA)
            {
                MakePrizeEmpty(2);
                MakePrizeCommand(PRIZE_DAY);
                PrizeDaya();
            ...
            //글라이더
            MakePrizeCommand(PRIZE_AIRPLANE);
            if (m_prizeInfo.m_nCont == JACK_AIRPLANE)
            {
                MakePrizeEmpty(2);
                MakePrizeCommand(PRIZE_DAY);
                PrizeAirPlane();
            ...
            //지구본
            MakePrizeCommand(PRIZE_EARTH);
            if (m_prizeInfo.m_nCont == JACK_EARTH)
            {
                MakePrizeEmpty(2);
                MakePrizeCommand(PRIZE_DAY);
                PrizeEarth();
            ...
            //모나리자
            MakePrizeCommand(PRIZE_MOZARINA);
            if (m_prizeInfo.m_nCont == JACK_MOZARINA)
            {
                MakePrizeEmpty(2);
                MakePrizeCommand(PRIZE_DAY);
                PrizeMozarina();
            ...
            //황금박쥐
            MakePrizeCommand(PRIZE_BAT);
            if (m_prizeInfo.m_nCont == JACK_GOLDBAT)
            {
                MakePrizeEmpty(2);
                MakePrizeCommand(PRIZE_DAY);
                PrizeGoldBat();
            ...
            if (m_prizeInfo.m_nCont == JACK_FIREBIRD)
            {
                MakePrizeEmpty(2);
                MakePrizeCommand(PRIZE_DAY);
                PrizeFBird();
        ...
        private void PrizeDaya()
        {
            List<CDVCScoreTable> lstTable = m_lstScoreTable.FindAll(value => value.m_nScore <= m_prizeInfo.m_nPrizeCash && value.m_nTile > 3).OrderByDescending(value => value.m_nScore).ToList();
            if (lstTable.Count == 0)
                return;

            CDVCScoreTable scoreTable = CGlobal.RandomSelect(lstTable.FindAll(value => value.m_nScore == lstTable[0].m_nScore));
            CDVCScoreInfo scoreInfo = ConvertScoreTableToScoreInfo(scoreTable);
        ...
        private void PrizeEarth()
        {
            List<CDVCScoreTable> lstTable = m_lstJackTable.FindAll(value => value.m_nScore <= m_prizeInfo.m_nPrizeCash).OrderByDescending(value => value.m_nScore).ToList();
            if (lstTable.Count == 0)
                return;

            CDVCScoreTable scoreTable = CGlobal.RandomSelect(lstTable.FindAll(value => value.m_nScore == lstTable[0].m_nScore));
            CDVCScoreInfo scoreInfo = ConvertScoreTableToPrizeScore(scoreTable);
            m_nReturnCash += (m_prizeInfo.m_nPrizeCash - scoreInfo.m_nScore);
        ...
        private CDVCScoreInfo ConvertScoreTableToPrizeScore(CDVCScoreTable scoreTable)
        {
            CDVCScoreInfo scoreInfo = new CDVCScoreInfo();
        ...
        private void MakeAfterPrizeScoreList(int nScore)
        {
            ...
            while (nScore >= 100)
            {
                ...
                CDVCScoreTable scoreTable = CGlobal.RandomSelect(m_lstScoreTable.FindAll(value => value.m_nScore == nCash));
                CDVCScoreInfo scoreInfo = ConvertScoreTableToScoreInfo(scoreTable);
                lstScoreInfo.Add(scoreInfo);
        ...
        public override void OnBroadCastPrizeKind()
        {
            ...
            {
                case PRIZE_EARTH:
                    strMessage = "[C][FFFFFF]운영자: " + strUserNick + " 님 [C][FFFF00]지구본[-] 출현을 축하드립니다. 대박 나세요..";
                    break;
                case PRIZE_MOZARINA:
                    strMessage = "[C][FFFFFF]운영자: " + strUserNick + " 님 [C][FFFF00]모나리자[-] 출현을 축하드립니다. 대박 나세요..";
                    break;
                case PRIZE_BAT:
                    strMessage = "[C][FFFFFF]운영자: " + strUserNick + " 님 [C][FFFF00]황금박쥐[-] 출현을 축하드립니다. 대박 나세요..";
                    break;
                case PRIZE_FBIRD:
        ...
        public override void OnGearStart(int nRun)
        {
            ...
            if (nRun == 1)
            {
                if (m_lstPrizeInfo.Exists(value => value.m_nCmd >= PRIZE_DAYA && value.m_nCmd <= PRIZE_FBIRD))
                {
                    CDVCScoreInfo scoreInfo = MakeEmptyRoll();
        ...
        public override void OnStartSpin(CUserSocket clsSocket, int nSpinCash = 100)
        {
            ...
            if (m_lstPrizeInfo.Count > 0)
            {
                ...
                if (m_bPrizeAni == false)
                {
                    ...
                    if (m_clsSendScoreInfo.m_nCmd == PRIZE_DAYA || m_clsSendScoreInfo.m_nCmd == PRIZE_AIRPLANE || m_clsSendScoreInfo.m_nCmd == PRIZE_EARTH ||
                        m_clsSendScoreInfo.m_nCmd == PRIZE_MOZARINA || m_clsSendScoreInfo.m_nCmd == PRIZE_BAT || m_clsSendScoreInfo.m_nCmd == PRIZE_FBIRD)
                    {
                        m_bPrizeAni = true;
                    }
                }
        ...
        private void ReturnGiftCash()
        {
            ...
            List<CDVCScoreInfo> lstOcaScoreInfo = new List<CDVCScoreInfo>();
        ...
        public override int RaiseJackPot1(int nJackCash)
        {
            ...
            m_lstPrizeInfo.Clear();
            List<CDVCScoreTable> lstScoreTable = m_lstScoreTable.ToList().FindAll(value => value.m_nScore <= nJackCash && value.m_nScore < 10000).OrderByDescending(value => value.m_nScore).ToList();
            if (lstScoreTable.Count > 0)
            {
                ...
                if (RND.Next(5) == 3)
                {
                    ...
                    SetDvcPrizeCall(nCont, nPrizeCash);
                }
                else
                {
                    CDVCScoreInfo scoreInfo = ConvertScoreTableToScoreInfo(CGlobal.RandomSelect(lstScoreTable));
                    m_lstPrizeInfo.Add(scoreInfo);
                    MakePrizeEmpty(1);
                }
        ...
        public override int RaiseJackPot2(int nJackCash)
        {
            ...
            CDVCPrizeCashInfo prizeCashInfo = m_lstPrizeCashInfo.FindAll(value => value.m_nCash <= nJackCash).OrderByDescending(value => value.m_nCash).ToList()[0];
            nJackCash -= prizeCashInfo.m_nCash;

            SetDvcPrizeCall(prizeCashInfo.m_nPrize, prizeCashInfo.m_nCash);
        ...
        public override int RaiseJackPotR(int nJackCash)
        {
            ...
            bool bret = SetDvcPrizeCall(prizeCashInfo.m_nPrize, prizeCashInfo.m_nCash);
            if (bret)
            {
        ...
        public CDVCPrizeInfo GetPrizeInfo()
        {
            return m_prizeInfo;
        }

        public List<CDVCScoreInfo> GetPrizeScoreInfoList()
        {
        ...
        public override void UseItemByUser(CItem clsItem)
        {
            ...
            int nJackCont = 0;
            if (clsItem.m_nItemModel == CItemModel.ITEM_DVC_EARTH)
            {
                nJackCont = JACK_EARTH;
            }
            else if (clsItem.m_nItemModel == CItemModel.ITEM_DVC_MONARIZA)
            {
                nJackCont = JACK_MOZARINA;
            }
            else if (clsItem.m_nItemModel == CItemModel.ITEM_DVC_BAT)
            {
                nJackCont = JACK_GOLDBAT;
            }
            

            List<CDVCPrizeCashInfo> lstPrizeCashInfo = m_lstPrizeCashInfo.FindAll(value => value.m_nPrize == nJackCont && value.m_nCash <= nItemCash);
            ...
            else
            {
                ...
                SetDvcPrizeCall(nJackCont, nJackScore);
