...
{
    public partial class CAldGear : CBaseGear
    {
        ...
        public override void OnAddSlotCash(CUserSocket clsSocket, int nCash)
        {
            ...
            if (m_nSlotCash < 100)
            {
                OnAddGiftCash(clsSocket, m_nSlotCash - 100, -1);
                m_nSlotCash = 0;
            }
            else
            {
        ...
        private void MakeFeverOpenPrizeRoll(List<CALDScoreInfo> lstPrizeInfo)
        {
            ...
            else if (nPrizeCash < 30 * 10000)
            {
                //두번에 나누어 준다.
                List<ALDScoreTableInfo> lstALDScoreTable0 = m_lstALDNormalScoreTable.ToList().FindAll(value => value.nGetScore >= 10 * 10000 && value.nGetScore <= 20 * 10000).ToList();
            ...
            {
                //세번에 나누어 준다.
                List<ALDScoreTableInfo> lstALDScoreTable0 = m_lstALDNormalScoreTable.ToList().FindAll(value => value.nGetScore <= 10 * 10000).ToList();
                ALDScoreTableInfo scoreTableInfo0 = lstALDScoreTable0[RND.Next(lstALDScoreTable0.Count)];
                CALDScoreInfo scoreInfo0 = ConvertScoreTableToScoreInfo(scoreTableInfo0);
                lstTempScoreInfo.Add(scoreInfo0);

                List<ALDScoreTableInfo> lstALDScoreTable1 = m_lstALDNormalScoreTable.ToList().FindAll(value => value.nGetScore >= 10 * 10000 && value.nGetScore <= 15 * 10000).ToList();
        ...
        {
            int nPrizeCash = m_prizeInfo.m_nJackCash;
            int nTempScore = nPrizeCash - 10000; //점수를 3 ~4번으로 갈라주기위해서이다.
        ...
        {
            int nPrizeCash = m_prizeInfo.m_nJackCash;
            int nTempScore = nPrizeCash - 10000; //점수를 3 ~ 4번으로 갈라주기 위한동작
        ...
        public override void UseItemByUser(CItem clsItem)
        {
            ...
            int nJackCont = 0;
            int nMinCash = 0;
            if (clsItem.m_nItemModel == CItemModel.ITEM_ALD_HORSE)
            {
                nJackCont = 1;
                nMinCash = 20000;
            }
            else if (clsItem.m_nItemModel == CItemModel.ITEM_ALD_YANGTAN)
            {
                nJackCont = 5;
                nMinCash = 20000;
            }
            else if (clsItem.m_nItemModel == CItemModel.ITEM_ALD_SMALL)
            {
                nJackCont = 2;
                nMinCash = 20000;
            }
            else if (clsItem.m_nItemModel == CItemModel.ITEM_ALD_LARGE)
            {
                nJackCont = 3;
                nMinCash = 50 * 10000;
            }

            if (nMinCash > nItemCash)
            {
                //가짜예시이다.
                if (RND.Next(2) == 0)
                    SetALDJackpotInfo(4, 0);
                else
                    SetALDJackpotInfo(6, 0);

                MakePrizeRoll();
                List<ALDScoreTableInfo> lstScoreTable = m_lstALDNormalScoreTable.ToList().FindAll(value => value.nGetScore <= nItemCash && value.nSlogan != 4
                && value.nSlogan != 14 && value.nSlogan != 24 && value.nSlogan != 34 && value.nSlogan != 44 && value.nSlogan != 54).OrderByDescending(value => value.nGetScore).ToList();
                if (lstScoreTable.Count > 0)
                {
                    int nScore = lstScoreTable[0].nGetScore;
                    ...
                    CDataBase.InsertNationalToDB(m_nGearCode, scoreInfo.m_nScore, 1);
                    CGlobal.SetItemEngineRemCash(nItemCash - scoreInfo.m_nScore);
                    m_nGearJack = 2;
                    m_lstAldPrizeInfo.Add(MakeEmptyOneRoll());
                }
            ...
            {
                //점수에 해당한 잭팟을 주어야 한다.
                SetALDJackpotInfo(nJackCont, nItemCash);
                MakePrizeRoll();
                CGlobal.SetItemEngineRemCash(nItemCash - m_nRealPrizeCash);

                CDataBase.InsertJackpotToDB(m_nGameCode, 0, m_nTakeUser, m_nGearCode, m_nRealPrizeCash, m_prizeInfo.m_nJackCont, 4, 0);
            }
        }
    }
}
