...
namespace ReelServer
{
    ...
    {
        public void PlayAldPrize()
        {
            Thread thr = new Thread(ProcessAldPrize);
            thr.Start();
        }

        private void ProcessAldPrize()
        {
            
            CAldGear gear = CGlobal.GetGearByCode(m_nGearCode) as CAldGear;
            if (gear == null)
                return;
                ...
                if (scoreInfo.m_nPrizeCmd > 0)
                {
                    switch (scoreInfo.m_nPrizeCmd)
                    {
                        ...
                        case CAldGear.ALD_YANGT_ANIMATION:
                            ...
                            break;
                        case CAldGear.ALD_FIRE_SMALL_ANIMATION:
                            ...
                            break;
                        case CAldGear.ALD_FIRE_LARGE_ANIMATION:
                            ...
                            break;
                        case CAldGear.ALD_FEVER_LOCK_ANIMATION:
                            ...
                            break;
                        case CAldGear.ALD_FEVER_OPEN_ANIMATION:
                            if (prizeInfo.m_nJackCont == 6 || prizeInfo.m_nJackCont == 7)
                            {
                                strMessage = "[C][FFFFFF]운영자: " + m_strRbNick + " 님 [C][D72D8D]휘버[-] 출현을 축하드립니다. 대박 나세요..";
                                CGlobal.SendNoticeBroadCast(strMessage);
